            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-8">
              {stats.map((stat, i) => (
                <div key={stat.label} className="text-center md:text-left">
                  <p className="text-4xl md:text-5xl font-bold text-highlight tracking-tight">
                    {stat.number}
                  </p>
                  <p className="text-sm font-medium text-foreground mt-2">
                    {stat.label}
                  </p>
                  <p className="text-xs text-muted-foreground mt-1.5 leading-relaxed max-w-[220px] mx-auto md:mx-0">
                    {stat.context}
                  </p>
                  {i < stats.length - 1 && (
