    ...
    <section className="py-16 md:py-20">
      <div className="max-w-5xl mx-auto px-4 md:px-8">
        <ScrollReveal delay={0}>
          <div className="rounded-2xl bg-indigo-950 border border-indigo-900/50 p-8 md:p-12">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-8">
              {stats.map((stat, i) => (
                <div key={stat.label} className="text-center md:text-left">
                  ...
                  <p className="text-xs text-indigo-400 mt-1.5 leading-relaxed max-w-[220px] mx-auto md:mx-0">
                    {stat.context}
                  </p>
                  {i < stats.length - 1 && (
                    <div className="md:hidden w-12 h-px bg-indigo-800 mx-auto mt-8" />
                  )}
                </div>
              ))}
            </div>
